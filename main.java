//Title: V4 AIO Cooking at Legends Guild
...
int npc_bank = 30087;
int cooking_range = 31631;
WorldPoint triForce = new WorldPoint(2465, 2848, 1); //triForce
...
private void bankingMethod(){
    ...
    if (v.getLocalPlayer().hasAnimation(-1) && (v.getInventory().amountInInventory(cookFood, 28 - counter, 28) || !v.getInventory().inventoryFull())) {
            GameObject bankChest = v.getGameObject().findNearest(npc_bank);
            if (bankChest != null) {
                int bankChestSceneX = bankChest.getSceneMinLocation().getX();
                int bankChestSceneY = bankChest.getSceneMinLocation().getY();

        v.invoke("Use","<col=ffff>Bank chest",npc_bank,3,bankChestSceneX,bankChestSceneY,false);    
...
private void cookingMania(){
   ...
   else if (currentLocation.equals(triForce) && v.getInventory().amountInInventory(rawFood, 28, 28))
    {
    	  GameObject cookingRange = v.getGameObject().findNearest(cooking_range);
    	  if (cookingRange != null) {
                int cookingRangeSceneX = cookingRange.getSceneMinLocation().getX();
                int cookingRangeSceneY = cookingRange.getSceneMinLocation().getY();
        v.invoke("Cook","<col=ffff>Fire",cooking_range,3,cookingRangeSceneX,cookingRangeSceneY,false);
